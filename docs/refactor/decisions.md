...
## 2026-05-27: Formal Training Uses Early Stopping On Composite Metric

- **Decision:** New formal training runs use `Seq2SeqTrainer` with `EarlyStoppingCallback` (patience=5, threshold=0.0) and `load_best_model_at_end=True`, driven by a composite metric `eval_composite = 0.5 · eval_bleu + 0.5 · eval_glossary_preservation_nospace`. `num_train_epochs` is the ceiling (default 10); `max_steps` is left unset so the early-stop loop decides when to halt. The earlier `max_steps=10000` profile (`configs/training/full_10k.json`, RF-006-P11) is preserved as a historical baseline and is not deleted.
- **Background:** RF-006-P11 used an arbitrary `max_steps=10000` (= 0.189 epoch). The resulting eval_loss was still decreasing and validation BLEU was monotonically rising at step 10000, indicating under-fit. A fixed step ceiling neither prevents under-fit (caps too early when the model still improves) nor over-fit (caps too late if val degrades earlier). Reviewed alternatives included plain post-hoc best-checkpoint selection, ReduceLROnPlateau, smoothed early stopping, learning-curve extrapolation, and Bayesian HPO; the patience + composite-metric + best-checkpoint combination was chosen as the standard NMT finetune practice that fits this project's stage.
- **Impact Scope:** `src/longtu_translation_pipeline/config.py` (TrainingArgumentsConfig + new MetricsConfig dataclass), `src/longtu_translation_pipeline/training.py` (Trainer → Seq2SeqTrainer switch with backward-compat fallback), new `src/longtu_translation_pipeline/training_metrics.py`, new `configs/training/full_earlystop.json`, `tests/test_training_pipeline.py`, RF-006-P13, RF-007-P4.
- **Follow-up Notes:** Composite weights (currently 0.5 / 0.5 on BLEU and preservation_nospace) can be revisited after observing the first early-stop run; adjustment requires a new RF, not silent edits. `predict_with_generate=True` makes each eval expensive — start with `generation_num_beams=1`. The `Trainer` (non-Seq2Seq) path stays available for smoke / pilot / `full_10k.json` so existing tests and old profiles keep working. Inference-time hyperparameter exploration (beam width, length penalty) belongs to RF-028, not this RF.
- **2026-05-27 follow-up — in-loop eval uses a 1,000-row validation subset.** The first T-A5 attempt measured 38 min per eval on the full 6,626-row validation set (predict_with_generate=True, batch=4, greedy). That cost was prohibitive across a 10-epoch ceiling. Per the 2026-05-25 8:1:1 split contract (seed=42, lock-in for RF-006-P11/RF-007-P3 reproducibility), the split itself stays at `train 53,015 / val 6,626 / test 6,626`. In-loop eval reads only the first 1,000 deterministic rows of `splits/validation.csv` via the new `metrics.eval_subset_rows` field. The remaining 5,626 rows are NOT sacrificed — they are reserved for post-hoc top-K full-validation (top 3 in-loop checkpoints re-evaluated on the full set) before T-A6. The standard NMT-finetune pattern: cheap subset for inner-loop decision, full validation for final selection. `generation_max_length` lowered to 256 for in-loop (verified on test_generated.csv that p99.9 of ko output tokens = 225, so 256 truncates ~0.06% of rows in-loop); the inference config `max_length=400` stays for post-hoc full-eval and T-A6 final test. Both changes apply only to the formal training path with `metrics.predict_with_generate=true`; smoke / pilot / RF-006-P11 baseline profile are unaffected.
