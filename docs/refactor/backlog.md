...
- **Status:** DONE
- **Scope:** `scripts/llm_common.py`, `scripts/segments_llm_cleanup_pipeline.py`, `scripts/glossary_llm_cleanup_pipeline.py`, `tests/test_llm_common.py`, `tests/test_segments_llm_cleanup_pipeline.py`, `tests/test_glossary_llm_cleanup_pipeline.py`, `docs/refactor/task-briefs/T-A1.md`, `docs/refactor/decisions.md`
...
- **Notes:** Decided and implemented 2026-05-27 as a precondition for T-A1. See `docs/refactor/decisions.md` "2026-05-27: LLM Cleanup Defaults To Batch API With Strict JSON Schema". T-A1 brief execution recipe rewritten to use the new default.
...
- **Notes:** Owned by [T-A5](task-briefs/T-A5.md). Decision rationale and methodology survey: see `docs/refactor/decisions.md` "2026-05-27: Formal Training Uses Early Stopping On Composite Metric". Gates T-A6 (RF-007-P4 new test report on the auto-selected best checkpoint).
...
- **Background / Why:** RF-006-P13 produces a single best-on-composite checkpoint (auto-loaded via `load_best_model_at_end=True`). This task runs the held-out test split against that checkpoint to produce the new project headline number. RF-007-P3 stays as the historical baseline number; this entry supersedes it as the current published result if the comparison favours the new run.
- **Concrete Scope:** Resolve the best checkpoint from `run-full-earlystop-v1/trainer_state.json` (`best_model_checkpoint`), run `--generate-test`, run `evaluate_translation.py`, write the report under `data/review/evaluation/test_report/run-full-earlystop-v1/<checkpoint>/`. Record comparison vs RF-007-P3 (ckpt-9000 of `run-full-10k-llm-segments-v1`) on all metrics (BLEU, glossary preservation exact + nospace, empty_candidate_rows).
- **Out of Scope:** Iterating over multiple checkpoints (data leakage); changing the selection rule (best-on-composite was set by RF-006-P13); rerunning training.
- **Risks:** If composite metric chose a checkpoint that turns out test-time worse than RF-007-P3 ckpt-9000, document the gap and decide whether to (a) ship RF-007-P3 as the production number, (b) revisit the composite weighting, or (c) accept the trade-off (e.g. higher preservation for slightly lower BLEU).
...
- **Notes:** Owned by [T-A6](task-briefs/T-A6.md) (brief to be written after RF-006-P13 completes — execution recipe depends on the auto-selected checkpoint path). Pending RF-006-P13.
