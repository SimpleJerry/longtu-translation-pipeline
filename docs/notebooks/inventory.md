...
本文档记录当前笔记本集合、各笔记本的存在理由及在重构工作中的处理建议。本文档首要目的是建立清单：历史实验在任何删除决定之前均需先归档。
...
- **2023-11-03：** `T&N+R` 的词汇表和代码保留准确率笔记本。
- **2023-11-28 至 2023-11-29：** `T&N+R preprocess` 修复及返回码 token 恢复实验。
- **2026-05-19：** 机械性 CSV 引用更新。许多笔记本仍引用已不再提交的中间文件。
- **2026-05-24：** RF-004 清单与归档整理。根目录笔记本已移入 `notebooks/main/`、`notebooks/analysis/` 和 `notebooks/archive/2023-legacy/`。

## 分类

### 主要笔记本

这些笔记本描述了历史主实验路径，在替代模块和 CLI 入口点存在之前应保持可查阅。2026-05-24 RF-005 决定后，T&N+R 笔记本已作为废弃历史实验处理，因为当前术语标记政策不再使用 `<middle>` 或 `<code_id=N>`。

| 笔记本 | 时间线 | 实验意义 | 当前依赖状态 | 建议 | 替代路径 |
| --- | --- | --- | --- | --- | --- |
...
| 笔记本 | 时间线 | 实验意义 | 当前依赖状态 | 建议 | 替代路径 |
| --- | --- | --- | --- | --- | --- |
| `notebooks/analysis/train_eval_loss_picture.ipynb` | 创建于 2023-10-26；更新于 2023-11-02。 | 用于可视化训练/评估损失曲线的工具笔记本。 | 依赖被忽略的微调模型输出下的本地 `trainer_state.json`。 | 保留为辅助分析记录。 | 若仍有用，RF-006 或 RF-007 可用小型报告脚本替代。 |

### 已归档历史笔记本

这些笔记本保留用于历史可溯源性，不应视为当前工作流。
...
| `notebooks/archive/2023-legacy/T&N method.ipynb` | 首次提交于 2023-09-06；演进至 2023-10。 | 使用词汇表特殊 token 的早期仅术语工作流。 | 使用当前 `data/glossary.csv`；引用未提交的旧版合并 CSV 文件。 | 归档，非主路径。 | 已被 `T&N+R` 笔记本及未来 RF-005 模块取代。 |
...
RF-004 第一轮整理中未删除任何笔记本。以下文件是后续仅删除任务的候选，待本清单审查后再处理：
...
- 多个笔记本仍引用已删除的中间语料文件，如 `all_files_merged.csv`、`all_files_merged_zh-CN_ko.csv`、已标记 CSV 输出、历史 `translation_result...csv` 文件及被忽略的训练输出。
- 本清单记录缺失引用，但不修复笔记本逻辑。路径修复、模块提取、配置迁移及自动化评估属于 RF-005、RF-006 和 RF-007 的工作范围。
