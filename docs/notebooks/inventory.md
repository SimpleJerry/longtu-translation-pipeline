...
These notebooks describe the historical main experiment path and should remain findable until replacement modules and CLI entry points exist. After the 2026-05-24 RF-005 decision, T&N+R notebooks are deprecated historical experiments because the current terminology marker policy no longer uses `<middle>` or `<code_id=N>`.
